...
url = 'http://match.yuanrenxue.com/api/match/4?page={}'
...
def base64_md5(str_):
    bs4 = base64.b64encode(str_.encode()).decode().replace('=', '')

    md = hashlib.md5()
    md.update(bs4.encode())
    display = md.hexdigest()
    print(display)
    return display


def css_left(num_style_list, num_list):
    num_style_list = [int(int(float(__)) / 11) for __ in num_style_list]
    # print(num_style_list)
    true_order = [None] * len(num_style_list)
    for index, num_style in enumerate(num_style_list):
        true_order[int(index + num_style)] = str(num_list[index])
        # print(true_order)
    return true_order
...
def get_data(page):
    data = []
    res = requests.get(url.format(page), headers=headers).json()
    key = res['key']
    value = res['value']
    info = res['info']
    k_v = base64_md5(key + value)
    td_list = re.findall(r'<td>(.*?)</td>', info)

    for td in td_list:
        # print(td)
        info_list = re.findall(r'src="data:image/png;base64,(.*?)"', td)
        display = re.findall(r'class="img_number (.*?)"', td)
        style = re.findall(r'style="left:(.*?)px"', td)
        ...
        for i in range(len(display)):
            if display[i] == k_v:
                continue
            num_list.append(img_number[info_list[i]])
            style_list.append(style[i])

        # print('数字列表：', num_list)
        # print('对应的偏移量：', style_list)
        data_list = css_left(style_list, num_list)
        # print('对应的数字正确位置：', data_list)
        num = int("".join(data_list))
        # print('数字：', num)
        data.append(num)
        # print("==" * 20)
    print('第{}页数字为：'.format(page), data)
    return data
...
def start():
    ...
    for page in range(1, 6):
        data_sum += get_data(page)
        time.sleep(0.5)

    print('5页数字总和为：', sum(data_sum))
